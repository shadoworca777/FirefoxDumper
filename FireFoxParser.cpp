...
String FireFoxParser::get_mozilla_program_dir(const String& temp_dir)
{
    ...
    char* raw_ini_copy = strdup(raw_ini.c_str());
    if (raw_ini_copy == nullptr) return "";

    try
    {
        const char* delimiter = "\r\n";
        char* pch = strtok(raw_ini_copy, delimiter);

        while (pch != nullptr)
        {
            char* sub_path = strstr(pch, "LastPlatformDir=");
            if (sub_path)
            {
                int start_index = strlen("LastPlatformDir=");
                int count = strlen(sub_path) - start_index;

                char* _temp_path = (char*)malloc(sizeof(char) * (count + 1));
                if (!_temp_path)
                {
                    break;
                }
...
cJSON* FireFoxParser::find_logins_node(cJSON* input_node, const char* pattern)
{
	...
	return find_logins_node(input_node->next, pattern);
}
